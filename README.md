...
さらにAppSheetを利用して請求一覧・未入金一覧・入金管理・売上分析機能を実装し、請求管理業務の効率化と、入金状況・売上状況の可視化を目的としたシステムとして開発。
...
- 請求データ追加・更新時に未入金データを検知
- 支払期限超過かつ入金日未入力の場合、メール通知を実行
- 定期スケジュール実行用Botも作成（AppSheet有料プラン利用時に有効化可能）
- 入金漏れ防止を支援
...
- 2026-07-23 売上分析機能を追加（対象年切替・年間売上集計）
- 2026-07-24 AppSheetアプリをDeploy状態へ移行
- 2026-07-24 AppSheet Automationの定期実行Botを作成（有料プラン利用時対応）
